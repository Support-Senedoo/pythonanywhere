...
**502-backend / site inaccessible après un utilitaire staff** : l’ancienne session Flask stockait **toute** la charge (flash, préférences) dans un **cookie signé**. Après certaines actions, l’en-tête `Set-Cookie` devenait trop gros pour la pile nginx/uWSGI → **502**. Sur PythonAnywhere, **`create_app`** active désormais des **sessions côté serveur** (`flask-session`, répertoire **`.flask_session/`** sous `odoo-pythonanywhere/`, hors Git). Seul un **identifiant** de session transite dans le cookie. Après la première mise à jour concernée : **`pip install -r requirements.txt`** sur PA, **Reload** Web ; les utilisateurs peuvent devoir **se reconnecter** une fois. En local, sans variable `PYTHONANYWHERE_*`, le cookie classique reste utilisé ; pour tester le mode fichiers : **`TOOLBOX_FILESYSTEM_SESSION=1`**.
