...
- **Balance 6 colonnes** : `/staff/utilities/personalize-balance` — [`personalize_balance_6cols.py`](personalize_balance_6cols.py). La copie est **`attach_to_root=False`** (rapport **autonome**, évite les `KeyError` Enterprise type `sn_open_deb` / trial balance). Le script **vide `root_report_id` en premier** (sans se fier au libellé du handler, variable en v19) et la duplication autonome **réinitialise aussi la racine** après recopie des options. Copie déjà en erreur : vider **Racine du rapport** sur l’`account.report` ou refaire une copie via la toolbox à jour. Après succès : création **`ir.actions.client`** + entrée **`ir.ui.menu`** sous **Analyse › Grands livres**, juste après **Balance comptable** (parent = celui du menu trial balance, xmlid typique `account_reports.menu_action_account_report_trial_balance` ; repli **Reporting** si introuvable), lien **`/web#menu_id=…&action=…`**, liste **`account.report`** en secours.
...
*Dernière mise à jour de cette section : avril 2026 — balance 6 col. : menu sous Analyse › Grands livres après Balance comptable ; détachement racine avant `sn_*` ; métadonnées base ; `deploy_pa.ps1 -SkipGitPush` ; Reload Web si pas de token API reload.*
