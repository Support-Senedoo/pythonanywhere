...
- **Balance 6 colonnes** : `/staff/utilities/personalize-balance` — [`personalize_balance_6cols.py`](personalize_balance_6cols.py). La copie est **`attach_to_root=False`** (rapport **autonome**, évite les `KeyError` Enterprise type `sn_open_deb` / trial balance). Le script **détache `root_report_id` en premier** (avant de recréer les colonnes `sn_*`). Copie déjà en erreur sur Odoo 19 SaaS : vider **Racine du rapport** sur l’`account.report` concerné ou refaire une copie via la toolbox. Après succès : création **`ir.actions.client`** + entrée **`ir.ui.menu`** sous **Reporting** (xmlid parent `account.account_reports_legal_statements_menu` ou repli `account.menu_finance_reports`), lien d’exécution **`/web#menu_id=…&action=…`**, lien liste **`account.report`** en secours.
...
*Dernière mise à jour de cette section : avril 2026 — balance 6 col. : détachement racine avant colonnes `sn_*` (KeyError `sn_open_deb`) ; autonome + menu Reporting + URLs ; métadonnées base (`odoo_instance_info.py`) ; `deploy_pa.ps1 -SkipGitPush` ; Reload Web si pas de token API reload.*
