...
- **CPC SYSCOHADA — Budget par projet (Senedoo)** : création / purge via [`archives-cli/create_cpc_budget_analytique.py`](archives-cli/create_cpc_budget_analytique.py) ; assistant Odoo (Studio / XML-RPC) [`create_cpc_odoo_wizard.py`](create_cpc_odoo_wizard.py). **Compte analytique** : choisi dans l’assistant *Budget par projet (Senedoo)*, pas via le filtre analytique du rapport (`filter_analytic=False` sur ce CPC). Après création ou mise à jour du rapport, bouton **Remplir le rapport CPC** (même période, même budget, même axe). **Pourquoi `external`** : le moteur Odoo ne fournit pas un croisement fiable *budget financier sélectionné × réalisé filtré par axe* sur une même colonne quand on s’appuie sur le filtre analytique standard du rapport ; la toolbox injecte donc réalisé et (si besoin) budget via `account.report.external.value`. **Limite** : en 18.0 / 19.0, le moteur *External Value* ne supporte pas le **groupby** par compte (dépliage) — les totaux par ligne CPC sont cohérents après remplissage ; le détail écriture par écriture reste le Grand livre ou un rapport `account_codes` dédié.
