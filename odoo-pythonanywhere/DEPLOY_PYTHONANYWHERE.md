...
- **Balance 6 colonnes** : `/staff/utilities/personalize-balance` — [`personalize_balance_6cols.py`](personalize_balance_6cols.py). La copie est **`attach_to_root=False`** (rapport **autonome**, évite les `KeyError` Enterprise type `sn_open_deb` / trial balance). Le script **détache `root_report_id` en premier** (avant de recréer les colonnes `sn_*`). Copie déjà en erreur sur Odoo 19 SaaS : vider **Racine du rapport** sur l’`account.report` concerné ou refaire une copie via la toolbox. Après succès : création **`ir.actions.client`** + entrée **`ir.ui.menu`** sous **Analyse › Grands livres**, juste après **Balance comptable** (parent = celui du menu trial balance, xmlid typique `account_reports.menu_action_account_report_trial_balance` ; repli **Reporting** si introuvable), lien **`/web#menu_id=…&action=…`**, liste **`account.report`** en secours.
