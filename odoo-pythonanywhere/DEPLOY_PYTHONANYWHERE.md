...
- **Balance 6 colonnes** : `/staff/utilities/personalize-balance` — [`personalize_balance_6cols.py`](personalize_balance_6cols.py). La copie est **`attach_to_root=False`** (rapport **autonome**, limite les `KeyError` Enterprise type `sn_open_deb` / trial balance). Le script **vide `root_report_id` et `section_main_report_ids`** (« Section de ») en premier, puis réécrit les options depuis la racine si besoin ; la duplication autonome fait de même après recopie des options. Réécriture **autonome** répétée en fin de script. **Handler trial balance** : **conservé** (sans lui, erreur « Méthode invalide `_report_custom_engine_trial_balance` » sur les lignes concernées). Copie déjà cassée (handler vide) : refaire une copie avec la toolbox à jour ou réaligner le handler sur la balance standard ; on peut aussi vider **Racine du rapport** / **Section de** depuis les rapports comptables. Après succès : création **`ir.actions.client`** + entrée **`ir.ui.menu`** dans le sous-menu **Grands livres** (parent = celui du **Grand livre général** si xmlid `menu_action_account_report_general_ledger` ; fin de liste des enfants ; repli après balance comptable puis **Reporting**), lien **`/web#menu_id=…&action=…`**, liste **`account.report`** en secours. Plus de confirmation « OUI » sur les formulaires copie (le bouton suffit).
