...
- **Balance 6 colonnes** : `/staff/utilities/personalize-balance` — [`personalize_balance_6cols.py`](personalize_balance_6cols.py). La copie est **`attach_to_root=False`** (rapport **autonome**, évite les `KeyError` Enterprise type `sn_open_deb` / trial balance). Le script **vide `root_report_id` et `section_main_report_ids`** (« Section de ») en premier, puis réécrit les options depuis la racine si besoin ; la duplication autonome fait de même après recopie des options. Réécriture **autonome** répétée en fin de script. Copie déjà en erreur : vider **Racine du rapport** sur l’`account.report` ou refaire une copie via la toolbox à jour. Après succès : création **`ir.actions.client`** + entrée **`ir.ui.menu`** dans le sous-menu **Grands livres** (parent = celui du **Grand livre général** si xmlid `menu_action_account_report_general_ledger` ; fin de liste des enfants ; repli après balance comptable puis **Reporting**), lien **`/web#menu_id=…&action=…`**, liste **`account.report`** en secours. Plus de confirmation « OUI » sur les formulaires copie (le bouton suffit).
